...
| **Ask** — a chatbot: a chat window with a header, messages oldest to newest, you on the right, the assistant on the left, a typing indicator, a pinned composer, and short memory so "and in the smelter?" follows on | SOW capability 7. The assistant never calculates; the rows are the proof. Without a key the page says so and nothing else breaks. |
